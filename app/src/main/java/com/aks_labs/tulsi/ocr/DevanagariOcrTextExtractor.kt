...
/**
 * Service class for extracting Devanagari text from images using ML Kit OCR
 */
class DevanagariOcrTextExtractor(private val context: Context) {
    ...
    /**
     * Extract Devanagari text from an image URI with enhanced device compatibility
     */
    suspend fun extractTextFromImage(imageUri: Uri): DevanagariOcrResult {
        return try {
            ...
            if (bitmap.isRecycled) {
                return DevanagariOcrResult.Error("Bitmap was recycled before processing")
            }

            if (bitmap.width <= 0 || bitmap.height <= 0) {
                return DevanagariOcrResult.Error("Invalid bitmap dimensions: ${bitmap.width}x${bitmap.height}")
            }
            ...
            val inputImage = try {
                ...
                return DevanagariOcrResult.Error("Failed to create input image: ${e.message}")
            }
            ...
            val result = try {
                ...
                Log.e(TAG, "Devanagari OCR processing failed", e)
                if (!bitmap.isRecycled) bitmap.recycle()
                return DevanagariOcrResult.Error("Devanagari OCR processing failed: ${e.message}")
            }
            ...
            Log.d(TAG, "Devanagari OCR completed successfully in ${processingTime}ms for image: $imageUri")
            Log.d(TAG, "Extracted text length: ${result.text.length}, blocks: ${result.textBlocks.size}")

            DevanagariOcrResult.Success(
            ...
            Log.e(TAG, "Devanagari OCR failed for image: $imageUri", e)
            DevanagariOcrResult.Error("Devanagari OCR processing failed: ${e.message}")
        }
    }
    
    /**
     * Extract Devanagari text from a bitmap directly with enhanced error handling
     */
    suspend fun extractTextFromBitmap(bitmap: Bitmap): DevanagariOcrResult {
        return try {
            val startTime = System.currentTimeMillis()

            Log.d(TAG, "Starting Devanagari OCR processing for bitmap: ${bitmap.width}x${bitmap.height}")

            // Validate input bitmap
            if (bitmap.isRecycled) {
                return DevanagariOcrResult.Error("Input bitmap is recycled")
            }
            ...
            val optimizedBitmap = try {
                ...
                return DevanagariOcrResult.Error("Failed to optimize bitmap: ${e.message}")
            }
            ...
            val result = try {
                ...
                Log.e(TAG, "Devanagari OCR processing failed for bitmap", e)
                if (optimizedBitmap != bitmap && !optimizedBitmap.isRecycled) {
                    optimizedBitmap.recycle()
                }
                return DevanagariOcrResult.Error("Devanagari OCR processing failed: ${e.message}")
            ...
            Log.d(TAG, "Devanagari OCR completed successfully in ${processingTime}ms for bitmap")
            Log.d(TAG, "Extracted text length: ${result.text.length}, blocks: ${result.textBlocks.size}")
            ...
            Log.e(TAG, "Devanagari OCR failed for bitmap", e)
            DevanagariOcrResult.Error("Devanagari OCR processing failed: ${e.message}")
        }
    }
    
    /**
     * Perform OCR using ML Kit Devanagari Text Recognition with timeout
     */
    private suspend fun performOcrWithTimeout(inputImage: InputImage) = withTimeoutOrNull(OCR_TIMEOUT_MS) {
        performOcr(inputImage)
    } ?: throw Exception("Devanagari OCR processing timed out after ${OCR_TIMEOUT_MS}ms")

    /**
     * Perform OCR using ML Kit Devanagari Text Recognition
     */
...
/**
 * Sealed class representing Devanagari OCR operation results
 */
sealed class DevanagariOcrResult {
    data class Success(
    ...
    data class Error(val message: String) : DevanagariOcrResult()
}
